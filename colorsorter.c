	...
	repeat (forever) {
		if (getDistanceValue(distanceMM) > 80) //moves only if length > 70
		{
			...
			else
			{
				...
				setMotor(clawMotor, -50); //claw drops object if not red
...
task Two()
{
	waitUntil(getBumperValue(port12) == 1);
	setMotor(leftMotor, -127);
	setMotor(rightMotor, -127);
	sleep(1000);//Wait for 1 second before continuing on in the program.
